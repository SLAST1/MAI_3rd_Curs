...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (recalculateFigure) {
            ...
            glad_glBufferData(VBO1, vertices.size() * sizeof(float), vertices.data(), GL_DYNAMIC_DRAW);
            glad_glBufferData(EBO1, indices.size() * sizeof(unsigned), indices.data(), GL_DYNAMIC_DRAW);
