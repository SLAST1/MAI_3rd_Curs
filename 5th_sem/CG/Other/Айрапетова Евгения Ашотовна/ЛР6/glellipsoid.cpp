...
#include <QVector3D>
GLSphere::GLSphere(float r, float p,float A, float B,float C) :
    radius(r),
    precision(p),    a(A),b(B),c(C)
{
...
void GLSphere::paint(){
    ...
    for(float phi = 0; phi < 2*M_PI;phi += step){
        ...
        for(float gma = M_PI - step; gma > 0; gma -= step){
            sinGma = sinf(gma);
            cosGma = cosf(gma);
            cur_point.setX(a* radius * sinGma * cosPhi);
            cur_point.setY(b* radius * sinGma * sinPhi);
            cur_point.setZ(c* radius * cosGma);
