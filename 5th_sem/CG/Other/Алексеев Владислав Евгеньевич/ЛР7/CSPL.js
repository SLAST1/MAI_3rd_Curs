...
CSPL._gaussJ = {};
CSPL._gaussJ.solve = function(A, x) { // Наши решения в матрице
    var m = A.length;
    for (var k = 0; k < m; k++) { // Столбец
        var i_max = 0;
    ...
    for (var i = m - 1; i >= 0; i--) { // Строки равны столбцам
        var v = A[i][m] / A[i][i];
        x[i] = v;
        for (var j = i - 1; j >= 0; j--) { // Строки
            A[j][m] -= A[j][i] * v;
            A[j][i] = 0;
        }
    }
